...
class Profile:
    ...
    def generate_sim(self, pairs):
       ...
       for (id1, id2) in pairs:
           for attribute in self.attributes:
              a1 = t1[t1['asin']==id1][attribute].tolist()[0]
              a2 = t2[t2['asin']==id2][attribute].tolist()[0]
              #print(a1, a2)
    ...
    def generate_stopcnt(self, pairs):
        ...
        for (id1, id2) in pairs:
           for attribute in self.attributes:
              for token in t1[t1['asin']==id1][attribute].tolist()[0].split():
                  if token in stop_words:
                      stop_cnt[attribute] += 1
              for token in t2[t2['asin']==id2][attribute].tolist()[0].split():
                  if token in stop_words:
                      stop_cnt[attribute] += 1
        return stop_cnt
