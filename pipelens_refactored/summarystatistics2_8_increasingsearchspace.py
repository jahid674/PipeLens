...
if __name__ == "__main__":
    ...
    sizes = [(2,2), (2,3),(3,3),(3,5), (5,5), (8,8)]
    sizes.reverse()

    f = sys.stdout
    if not(os.path.exists('ERmetrics/experiment2summary_8.txt')):
      f = open('ERMetrics/experiment2summary_8.txt', 'w')
    f.write("experiment 2 summary statistics")
    f.write('\n')

    profile_data = []
    for size in sizes:
      search_size = 8 * size[0] * (size[1] + 1) if size[1] < 4 else 8 * size[0] * size[1]
      f_goal = 0.95
      ...
      if 0.725 not in param.ranges[3] and size[1]<8:
         param.ranges[3].append(0.725)
      ...
      if 0.725 not in projparam.ranges[3] and size[1]<8:
         projparam.ranges[3].append(0.725)
      ...
      if 0.725 not in profile.ranges[3] and size[1]<8:
         profile.ranges[3].append(0.725)
          ...
          for param2 in np.linspace(0.05, 0.95, size[0]).tolist():
              for param3 in list(range(4)):
                for param4 in np.linspace(0.05, 0.95, size[1]).tolist():
                    ...
                    profile_data.append((param1, round(param2, 5), param3, round(param4, 5), search_size, param.rank_iter, param.rank_f, projparam.rank_iter, projparam.rank_f, profile.rank_iter, profile.rank_f, round(g_iquartiles[1],5), round(g_fquartiles[1],5)))
      ...
      print("search space size stats: " + str(search_size))
      f.write("stats for search space size: " + str(search_size))
      f.write('\n')
    ...
    f.close()
    profiles_df = pd.DataFrame.from_records(profile_data, columns=['init pf','init block clean threshold', 'init weighting schema', 'init matching threshold', 'search space size', 'param iterations', 'param f-score', 'proj iterations', 'proj f-score', 'profile iterations', 'profile f-score', 'grid search iterations', 'grid search f-score'])
    if not(os.path.exists('ERmetrics/experiment2summary_8.csv')):
      profiles_df.to_csv('ERmetrics/experiment2summary_8.csv')
