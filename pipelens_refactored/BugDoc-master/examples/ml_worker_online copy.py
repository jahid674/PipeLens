...
def split_params(params, values):
    ...
    - strategy: base step names (EXCLUDES 'model'), values are '1'..'n'
...
def compute_order(steps, positions, baseline_order):
    ...
    baseline_order: reference order to break ties
    Returns ordered list of base step names.
    """
...
while True:
    ...
    try:
        ...
        # 2) derive baseline order from the present strategy keys (exclude model/pos_)
        baseline_order = []
        for k in parameters:
            k_str = str(k)
            if k_str.startswith("pos_"):
        ...
        # 3) compute evaluation order using positions (stable by baseline)
        ...
        # 5) append model at the end for evaluation (fixed)
        eval_order.append('model')
        eval_values.append(int(DEFAULT_MODEL_STRAT))

        # 6) evaluate
        TOTAL_EVALS += 1
        print(f"[WORKER] total_pipeline_exec={TOTAL_EVALS} | eval_order={eval_order} | eval_values={eval_values}")
    ...
    # Record exactly what BugDoc sent (do NOT rewrite keys)
