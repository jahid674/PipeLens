...
class OptimizingParam:
    def __init__(self):
        ...
        self.p1=pd.read_csv('DBLP-ACM/DBLP2.csv',encoding="latin-1")
        self.p2=pd.read_csv('DBLP-ACM/ACM.csv')

        self.p1sample = pd.read_csv('DBLP-ACM/DBLP2_Sample.csv',encoding="latin-1")
        self.p2sample = pd.read_csv('DBLP-ACM/ACM_Sample.csv')

        self.t1=pd.read_csv('DBLP-ACM/noisy20.csv',encoding="latin-1")
        self.t2=pd.read_csv('DBLP-ACM/noisy21.csv')
        ...
        self.passing_profile.generate_bbprofiles([self.p1sample, self.p2sample], ['title'])
        self.failing_profile.generate_bbprofiles([self.t1sample, self.t2sample], ['title'])
        ...
        self.ranges[0] = [0,4,5]
        self.ranges[1] = [0,1]
        self.ranges[2] = np.linspace(0.05, 0.95,self.bstep_size).tolist()
        self.ranges[3] = list(range(4))
        self.ranges[4] = np.linspace(0.05, 0.95,self.mstep_size).tolist()
    ...
    def ranking(self):
      ...
      if not(os.path.exists('ERmetrics/passing_historical.csv')):
        profile_data = []
        q_lst = [0,4,5]
        for q in q_lst:
          for i in range(2):
              for j in np.linspace(0.05, 0.95, 4).tolist():
                  ...
                  for k in range(4):
                      for l in np.linspace(0.05, 0.95, 4).tolist():
                          ...
                          (_, _, _, _, _, _, cur_f) = self.pipeline(params, True)
        ...
        profiles_df.to_csv('ERmetrics/passing_historical.csv')
        #print(os.path.exists('ERmetrics/parameter_ranking.csv'))

      profiles_df = pd.read_csv('ERmetrics/passing_historical.csv')
    ...
    def rank_profiles(self):
      if not(os.path.exists('ERmetrics/passing_historical_profiles.csv')):
        profile_data = []
        q_lst = [0,4,5]
        for q in q_lst:
          for i in range(2):
            for j in np.linspace(0.05, 0.95, 4).tolist():
                ...
                for k in range(4):
                    for l in np.linspace(0.05, 0.95, 4).tolist():
                        matching_thres = round(l,5)
                        params = [q, i, block_clean_thres, k, matching_thres]
                        (stop_cnt, av_sim, sim_q1, sim_q2, sim_q3, sample_f, _) = self.pipeline(params, True, True)
                        (_, _, _, _, _, _, cur_f) = self.pipeline(params, True, False)
                        profile_data.append((q, i, block_clean_thres, k, matching_thres, stop_cnt, av_sim, sim_q1, sim_q2, sim_q3, sample_f, cur_f))
       
        profiles_df = pd.DataFrame.from_records(profile_data, columns=['q size', 'pf','block clean threshold', 'weighting schema', 'matching threshold', 'stop count', 'av sim', 'sim q1', 'sim q2', 'sim q3', 'sample f-score', 'f-score'])
        profiles_df.to_csv('ERmetrics/passing_historical_profiles.csv')
        
      profiles_df = pd.read_csv('ERmetrics/passing_historical_profiles.csv')
      print("beginning profile regression generation")
      X = profiles_df[['stop count', 'av sim', 'sim q1', 'sim q2', 'sim q3', 'sample f-score']]
      y = profiles_df['f-score']
      reg = Regression()
      model = reg.generate_regression(X, y)
      coefs = model.coef_
    ...
    def init_profile_searchspace(self):
       if not(os.path.exists('ERmetrics/failing_profiles1.csv')):
        profile_data = []
        for q in self.ranges[0]:
          for i in self.ranges[1]:
            for j in self.ranges[2]:
                block_clean_thres = round(j,5)
                for k in self.ranges[3]:
                    for l in self.ranges[4]:
                        matching_thres = round(l,5)
                        params = [q, i, block_clean_thres, k, matching_thres]
                        (stop_cnt, av_sim, sim_q1, sim_q2, sim_q3, sample_f, _) = self.pipeline(params, False, True)
                        (_, _, _, _, _, _, cur_f) = self.pipeline(params, False, False)
                        profile_data.append((q, i, block_clean_thres, k, matching_thres, stop_cnt, av_sim, sim_q1, sim_q2, sim_q3, sample_f, cur_f))
       
        profiles_df = pd.DataFrame.from_records(profile_data, columns=['q size', 'pf','block clean threshold', 'weighting schema', 'matching threshold', 'stop count', 'av sim', 'sim q1', 'sim q2', 'sim q3', 'sample f-score', 'f-score'])
        profiles_df.to_csv('ERmetrics/failing_profiles1.csv')
    ...
    def rank_parameter(self):
      ...
      #   print(coefs)
      #   print(model.intercept_)
        self.parameter_rank[elem] = np.argsort(np.abs(coefs))[::-1]
    ...
    def failing_rank_parameters(self):
        ...
        for elem in self.profiles:
         ...
         #  print(coefs)
         #  print(model.intercept_)
          ranking[elem] = np.argsort(np.abs(coefs))[::-1]
          parameter_coefs[elem] = coefs
         #  print(elem + ':')
         #  print(ranking[elem])
    ...
    def profile_memoization_optimize(self, init_params, f_goal):
       ...
       while(opt_index < search_size):
        for idx, profile_index in enumerate(self.profile_ranking):
          opt_profile = profiles_df.loc[(profiles_df['q size'] == opt_params[0]) & 
                                (profiles_df['pf'] == opt_params[1]) & 
                                (profiles_df['block clean threshold'] == opt_params[2]) &
                                (profiles_df['weighting schema'] == opt_params[3]) &
                                (profiles_df['matching threshold'] == opt_params[4])].iloc[0][self.profiles[profile_index]]

          if(opt_index == 0 or idx != 0):
           for elem in self.parameter_rank[self.profiles[profile_index]]:
             cur_params = list(opt_params)
             test_params = cur_params.copy()
             #want maximal profile value
             if self.profile_coefs[profile_index] > 0:
    ...
    def pipeline(self, params, passing, profile_flag):
        pf = False if params[1] == 0 else True
        BlCl = BlockCleaning(pf, params[2])
        CoCl = ComparisonCleaning(params[3])
        if profile_flag:
         pairs = CoCl.generate_pairs(BlCl.clean_blocks(self.sample_blocks[params[0]])) if passing else CoCl.generate_pairs(BlCl.clean_blocks(self.sample_failing_blocks[params[0]]))
        else:
        ...
        if len(pairs) > 0:
          if profile_flag:
            ...
            Jm = Matching(params[4])
            (tp,fp,tn,fn) = Jm.pair_matching(pairs,[self.p1,self.p2],self.gt_list) if passing else Jm.pair_matching(pairs,[self.t1,self.t2],self.gt_list)
            #print((tp, fp, tn, fn))
            if tp == 0:
...
if __name__ == "__main__":
   ...
   #  print("end testing")
    ...
    print(p.rank_iter)
    print(p.rank_f)
    opt_params = p.profile_memoization_optimize([0,0,0.35,2,0.65], 0.90) #22 0.9311 [4,0,0.35,1,0.65]
    print(opt_params)
    print(p.rank_iter)
    print(p.rank_f)
    ...
    print("f-goal of 0.93")
    print("default")
    opt_params = p.optimize([0,0,0.35,2,0.65], 0.90)
    print(opt_params)
    print(p.rank_iter)
    print(p.rank_f)
    opt_params = p.profile_memoization_optimize([0,0,0.35,2,0.65], 0.93) #22 0.9311 [4,0,0.35,1,0.65]
    print(opt_params)
    print(p.rank_iter)
    print(p.rank_f)
   ...
   #  plt.show()
    ...
    profiles_df = pd.DataFrame.from_records(p.trace_data, columns=['q size', 'pf','block clean threshold', 'weighting schema', 'matching threshold', 'cur_f'])
    if not(os.path.exists('ERmetrics/profilefailingtrace.csv')):
         profiles_df.to_csv('ERmetrics/profilefailingtrace.csv')
