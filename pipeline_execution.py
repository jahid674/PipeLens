...
def compute_similarity(v1, v2, metric='cosine'):
    """Compute profile similarity for glass-box intervention ranking."""
    ...
    if metric == 'euclidean':
        return 1 / (1 + euclidean(v1.flatten(), v2.flatten()))
    if metric == 'manhattan':
        return 1 / (1 + cityblock(v1.flatten(), v2.flatten()))
    if metric == 'pearson':
        correlation, _ = pearsonr(v1.flatten(), v2.flatten())
        return correlation
...
class PipelineExecutor:
    def __init__(self, pipeline_type, dataset_name, metric_type, pipeline_ord,
        ...
        self.noise_types = ["missing", "outlier", "class_imbalance"]
        self.noise_params = {
            "invalid_value": {"frac": 0.10},
            "missing": {"frac": 0.10, "col": None},
            "outlier": {"frac": 0.03},
            "class_imbalance": {"frac_random": 0.03},
            # "floating_point": {
            #     "noise_scale": 1e-6,
            #     "frac_rows": 0.50
            # },
    ...
    def run_pipeline_opaque_sample(self, file_name, n=1000, seed=42):
        """
        Randomly sample n unique pipeline parameter combinations and evaluate them
        using the opaque pipeline setting.

        Output CSV columns:
            eval_order components + utility column

        Example:
            missing_value, normalization, outlier, model, utility_sp
        """

        # ---------- 0) Load existing ----------
        if os.path.exists(file_name):
            print(f"[OpaqueRandom] File already exists -> skipping generation: {file_name}")
            self.param_lst_df = pd.read_csv(file_name)
            return self.param_lst_df

        t0 = time.perf_counter()
        rng = np.random.default_rng(seed)

        # ---------- 1) Enforce valid pipeline order + model last ----------
        ...
        # ---------- 2) Data setup same as run_pipeline_opaque ----------
        if self.execution_type == 'pass':
            X_copy, y_copy = self.X_train.copy(), self.y_train.copy()
            _, _, sensitive_attr_train = self.getIdxSensitive(X_copy, self.sensitive_var)
        else:
        ...
        if self.pipeline_type != 'ml':
            raise ValueError("run_pipeline_opaque_sample currently assumes pipeline_type == 'ml'.")

        # ---------- 3) Build sampling space ----------
        ...
        while len(sampled) < n and attempts < max_attempts:
            ...
            # 0-based values for handlers
            combo_0b = [int(rng.integers(0, sizes[i])) for i in range(len(sizes))]

            # 1-based key for uniqueness and CSV-style representation
        ...
        # ---------- 5) Evaluate sampled combinations ----------
        param_lst = []
        utility_col = f'utility_{self.metric_type}'

        print(f"[OpaqueRandom] Evaluating {len(sampled)} random combinations with seed={seed}")
        print(f"[OpaqueRandom] eval_order: {eval_order}")
        ...
        for run_idx, combo_0b in enumerate(sampled, start=1):
            ...
            for step_i, step in enumerate(eval_order):
                ...
                # store 1-based strategy index in CSV
            ...
            if run_idx % 25 == 0 or run_idx == 1 or run_idx == len(sampled):
                print(
                    f"[OpaqueSample] {run_idx}/{len(sampled)} | "
                    f"params={param_record} | utility={utility}"
                )

        # ---------- 6) Save output ----------
        cols = eval_order + [utility_col]
        ...
        elapsed = time.perf_counter() - t0
        print(
            f"[run_pipeline_opaque_sample] wrote {len(df_new)} rows to {file_name} "
            f"| runtime={elapsed:.3f}s"
        )

        try:
            logging.info(f"run_pipeline_opaque_sample_runtime_seconds={elapsed:.6f}")
        except Exception:
            pass

        self.param_lst_df = df_new
        return df_new
    ...
    def evaluate_interventions_pred_and_similarity(self, cur_par, filename_training, new_components, wS=1.0, wU=1.0):
        ...
        # sort by similarity (as you are doing)
        fused_ranking.sort(key=lambda x: x[3])
        ...
        # save to CSV
        df.to_csv("fused_ranking.csv", index=False)
