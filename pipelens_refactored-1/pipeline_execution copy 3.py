...
# Single sources of truth (you only edit these files when strategies/thresholds change)
from config_strategies import build_strategies, build_strategy_counts, build_shared_config
from config_threshold import get_thresholds
...
class PipelineExecutor:
    def __init__(self, pipeline_type, dataset_name, metric_type, pipeline_ord,
        ...
        self.pipeline_order = pipeline_ord  # e.g., ['missing_value','normalization','model']
        ...
        if self.pipeline_type == 'ml':
            # ---------- Strategies & counts come ONLY from config_strategies ----------
            # Change strategies in config_strategies.py and nowhere else.
            self.strats = build_strategies(self.pipeline_type)
            self.strategy_counts = build_strategy_counts(self.strats)

            # Keep optional attributes for downstream compatibility if used anywhere
            # (e.g., rank_profile_new_comp had a conditional on model type)
            self.model_selection = self.strats.get('model', ['lr'])
            ...
            self.h_sample = None
            if self.h_sample_frac is not None and 0 < self.h_sample_frac < 1.0:
                self.h_sample = self.h_sample_frac
            ...
            # ---------- Dataset thresholds from config_thresholds ----------
            th = get_thresholds(self.dataset_name, self.execution_type)
            self.tau = th['tau']
            self.contamination = th['contamination']
            self.contamination_lof = th['contamination_lof']

            # ---------- Shared config built ONLY from strategies + thresholds ----------
            self.shared_config = build_shared_config(
                self.strats,
                metric_type=self.metric_type,
                sensitive_var=self.sensitive_var,
                target_var=self.target_variable_name
            )
            # Make thresholds available to handlers exactly as before
            self.shared_config['contamination'] = self.contamination
            self.shared_config['contamination_lof'] = self.contamination_lof
    ...
    def fuse_scores(self, similarity, utility, method='geometric', wS=1.0, wU=1.0, normalize=True):
        ...
        if normalize:
            S_hat = _minmax(S)
            U_hat = 1.0 - _minmax(U)  # lower utility is better → invert
        else:
            U_pos = U - np.min(U)
    ...
    def rank_profile_new_comp(self, file_name, new_comp):
        ...
        # Mirror previous default path (model list contains 'lr' only)
    ...
    def evaluate_interventions_pred_and_similarity(self, cur_par, filename_training, new_components, wS=1.0, wU=1.0):
        ...
        # ensure rank_profile exists (kept as-is)
        try:
        ...
        for i, component in enumerate(original_order):
            ...
            for new_strategy in range(1, num_strategies + 1):
                ...
                fused_rows.append((component, new_strategy, float(sim), float(y_pred), None, y_truth))

        # B) New components: best position by similarity
        for comp in new_components:
            ...
            for strat_idx in range(comp_ranges):
                ...
                for insert_pos in insertion_positions:
                    ...
                    y_pred = _predict_from_row_df(row_df)
                    y_truth = row_df[f'utility_{self.metric_type}'].values[0]
                    if (best_for_this_strat is not None) and (sim <= best_for_this_strat[0]):
                        continue
                    best_for_this_strat = (float(sim), float(y_pred), insert_pos, y_truth)

                if best_for_this_strat is not None:
                    sim_star, ypred_star, pos_star, _ = best_for_this_strat
                    fused_rows.append((comp, strat_idx + 1, float(sim_star), float(ypred_star), pos_star, y_truth))
        ...
        fused_ranking.sort(key=lambda x: x[2], reverse=True)  # current behavior: sort by similarity only

        for idx, (component, strategy, sim, y_pred, pos, y_truth, _) in enumerate(fused_ranking, start=1):
            if pos is not None:
