...
class Translation:
    ...
    def translate(self, text):
        if self.flag:
            language = detect(text, languages=["de", "fr", "en"])[0]
            print(language)
            return argostranslate.translate.translate(text, language, "en")
        else:
            return text
...
class BlockBuilding:
    ...
    def create_blocks_from_dataframe(self, df_lst, attribute_lst, translate = False):
        ...
        for attribute in attribute_lst:
            ...
            for df in df_lst:
                for index,rec in df.iterrows():
                    self.add_record(translator.translate(rec[attribute]),rec['asin'],iter)
                iter+=1
        return self.blocks
...
if __name__ == "__main__":
      t1=pd.read_csv('AmazonData/historical/original_sample.csv',encoding="latin-1")
      t2=pd.read_csv('AmazonData/historical/noisy_sample.csv',encoding="latin-1")

      gt = pd.read_csv('AmazonData/historical/asin_mapping.csv')
