...
class base:
                ...
                while(iter_size<3):
                        if iter_size == 1:
                                ...
                                for idx,profile_index in enumerate(self.profile_ranking):
                                        ...
                                        # profile_index = 2
                                        # cur_params_opt = {strategy: selection for strategy, selection in zip(self.base_strategies, init_params[:len(self.base_strategies)])}
                                        opt_profile = self.get_profile(self.historical_data_pd,list(cur_params_opt.values()),self.profiles[profile_index])
                                        ...
                                        if True:
                                                ...
                                                for val in coef_rank:
                                                        ...
                                                        for param_val  in current_paramter:
                                                                ...
                                                                is_opt_profile_found = False
                                                                if(self.profile_coefs[profile_index]< 0):
                                                                        is_opt_profile_found =  curr_profile > opt_profile
                                                                ...
                                                                self.rank_iter += 1
                                                                logging.info(f'Is Optimal profile found : {is_opt_profile_found}')
                                                                if True:
                                ...
                                opt_f  = self.f_score_look_up2(self.historical_data_pd,init_params)
                                for idx,profile_index in enumerate(self.profile_ranking):
                                #opt_profile = self.get_profile(self.historical_data_pd,cur_params,self.profiles[profile_index])
                                        profile = self.profiles[profile_index]
                                        sorted =[]
                                        if(self.profile_coefs[profile_index]>0):
                                                sorted = self.historical_data_pd.sort_values(by=profile, ascending=False)
                                        else:
                                                sorted = self.historical_data_pd.sort_values(by=profile, ascending=True)
                                        
                                        pd_index = sorted.index[iter_size]
                                        # if cur_params not in seen:
                                        #         self.rank_iter += 1
                                        #         seen.add(cur_params)
                                        param  = {'missing_value':self.historical_data_pd.loc[pd_index]['missing_value'],'normalization':self.historical_data_pd.loc[pd_index]['normalization'],'outlier':self.historical_data_pd.loc[pd_index]['outlier']}
                                        cur_f = self.f_score_look_up2(self.historical_data_pd,list(param.values()))
                                        if(tuple(param.items())) in seen:
                                                continue
                                        self.rank_iter += 1
                                        seen.add(tuple(param.items()))
                                        if cur_f <= f_goal:
                                ...
                                iter_size +=1
                                # print('Failed for 1 step')
                                self.fail = self.fail  + 1
                                # break
                                
                        else:

                                self.fail_grid_search = self.fail_grid_search  + 1
                                print(f'Failing seed {init_params} ')
        ...
        def create_historic_data(self,file_name):
                ...
                if not(os.path.exists(file_name)):
                        ...
                        for param1 in range(mv_params):
                                for param2 in range(norm_params):
                                        ...
                                        # print("Dropping missing values ...")
                                                ...
                                                sens_data =[fraction_out,diff_sensitive_attr,ratio_sensitive_attr,cov]
                                                ...
                                                param_lst.append(mv_param + norm_param + od_param + sens_data + f)
                                                # if eqOdds_train == 0.0:
                        ...
                        param_lst_df = pd.DataFrame(param_lst, columns= param_column   + ["profile_outlier","diff_sensitive_attr","ratio_sensitive_attr","cov","fairness"])
                ...
                self.profiles = [['profile_outlier','diff_sensitive_attr','ratio_sensitive_attr']]
                        ...
                        for param1 in range(mv_params):
                                for param2 in range(norm_params):
                                        ...
                                        # print("Dropping missing values ...")
                                                ...
                                                diff_sensitive_attr = round(y_pred_priv - y_pred_unpriv,5)
                                                ratio_sensitive_attr =  round(len(concat_X_y[(concat_X_y[sens_attr_name] == 1)] )/len(concat_X_y[(concat_X_y[sens_attr_name] == 0)]),5)
                                                cov = concat_X_y[sens_attr_name].cov(concat_X_y[outlier_y_test.name])
                                                sens_data =[fraction_out,diff_sensitive_attr,ratio_sensitive_attr,cov]
...
for f_goal in f_goals:
        ...
        # random.shuffle(gg)
        failures = 0
        # gg = [[1.0, 3.0, 1.0, 0.0, 0.046, 5.14865, 0.0477406540398666]]
        for seed_ in gg:
                seen = set()
