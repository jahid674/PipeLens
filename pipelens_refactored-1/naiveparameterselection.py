...
class Param:
    ...
    def init_searchspace(self):
       if not(os.path.exists('ERmetrics/parameter_ranking2.csv')):
        profile_data = []
        for i in range(2):
        ...
        profiles_df.to_csv('ERmetrics/parameter_ranking2.csv')
        #print(os.path.exists('ERmetrics/parameter_ranking.csv'))

    def ranking(self):
      if not(os.path.exists('ERmetrics/ranking_dataset.csv')):
        profile_data = []
        for i in range(2):
        ...
        #print(os.path.exists('ERmetrics/parameter_ranking.csv'))
        profiles_df = pd.DataFrame.from_records(profile_data, columns=['pf','block clean threshold', 'weighting schema', 'matching threshold', 'f-score'])
        profiles_df.to_csv('ERmetrics/ranking_dataset.csv')
        #print(os.path.exists('ERmetrics/parameter_ranking.csv'))

      profiles_df = pd.read_csv('ERmetrics/ranking_dataset.csv')
      print("beginning regression generation")
      X = profiles_df[['pf', 'block clean threshold', 'weighting schema', 'matching threshold']]
       ...
       while (iter_size < len(self.rank)):
         ...
         #  cur_params = init_params.copy()
          ...
          if iter_size == 1:
            ...
            #for each iter_size
            while index < len(self.rank):
               cur_iterlst = []
               for i in range(iter_size):
                  if (index >= len(self.rank)):
                     break
                  cur_iterlst.append(self.ranges[self.rank[index]])
                  index += 1
               for elem in product(*cur_iterlst):
                  test_params = cur_params.copy()
                  for j in range(len(cur_iterlst)):
                     test_params[self.rank[index - len(cur_iterlst) + j]] = round(elem[j], 5)
                  #account for hashing
                  if tuple(test_params) not in seen:
                     self.rank_iter += 1
                     seen.add(tuple(test_params))
                  ...
                  if cur_f >= f_goal:
                     for j in range(len(cur_iterlst)):
                        cur_params[self.rank[index - len(cur_iterlst) + j]] = round(elem[j], 5)
                     self.rank_f = cur_f
                     return cur_params # early exit when f_goal obtained
                  # update opt f for current iteration to improve upon in next iter
                  elif cur_f > opt_f:
...
if __name__ == "__main__":
   ...
   #  print(p.rank_f)
    ...
    data_df = pd.read_csv('ERmetrics/parameter_ranking2.csv')
