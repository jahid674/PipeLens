...
def compute_order(steps, positions, baseline_order):
    ...
    # compute key: (pos, baseline_index, name) with defaults if missing
    ordered = sorted(
        steps,
...
while True:
    ...
    try:
        ...
        # 2) derive baseline order from the present strategy keys (model not present here)
        baseline_order = [strip_suffix(k) for k in parameters if not str(k).startswith("pos_")]
        ...
        # 5) append model at the end for evaluation
