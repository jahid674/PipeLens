...
if __name__ == "__main__":
    summary_df = pd.read_csv('metric/scalability/goal_0.06/scalability_results_summary.txt')
    summary_group = summary_df.groupby(['method','sample_size'])
    s_sizes = [315, 400, 600, 840]
    ...
    plt.legend(fontsize=fsize/1.2, loc='center left')

    plt.xlabel('Search space size',labelpad=0, fontsize=fsize/1.2)
    plt.ylabel('#iterations',labelpad=0, fontsize=fsize/1.2)
    plt.savefig('MLfigures/searchspace_q4.pdf')