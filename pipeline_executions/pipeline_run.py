...
class PipelineExecutor:
    ...
    def computeStatisticalParity(self, p_priv, p_unpriv):
        return p_priv.mean() - p_unpriv.mean()
    ...
    def run_pipeline(self, file_name, X_train, y_train, sensitive_attr_train,
                     pipeline_order=['mv', 'norm', 'od', 'model']):
        param_lst_df = None
        if os.path.exists(file_name):
            return pd.read_csv(file_name)[pipeline_order + ["fairness"]]
        else:
                ...
                for param2 in range(norm_params):
                    for param3 in range(od_params):
                        for param4 in range(model_params):
                            ...
                            sensitive_processed = sensitive_attr_train.copy() if sensitive_attr_train is not None else None
                                ...
                                if step == 'mv':
                                    ...
                                    if param3 < len(od_strategy) - 1:
                                        ...
                                        if od_choice == 'none':
                                            ...
                                            X_processed, y_processed, sensitive_processed, priv_idx_train, unpriv_idx_train = outlier_detector.transform(
                                                                                                y_processed, sensitive_processed
                                                                                        )
                                            
                                        elif od_choice == 'if':
                                            outlier_detector = OutlierDetector(X_processed, strategy=od_choice, contamination=contamination_train, verbose=False)
                                            X_processed, y_processed, sensitive_processed, priv_idx_train, unpriv_idx_train = outlier_detector.transform(
                                                                                                y_processed, sensitive_processed
                                                                                        )
                                        ...
                                        outlier_detector = OutlierDetector(X_processed, strategy='lof', k=k, contamination=contamination_train_lof, verbose=False)
                                        X_processed, y_processed, sensitive_processed, priv_idx_train, unpriv_idx_train = outlier_detector.transform(
                                                                                        y_processed, sensitive_processed
                                                                                )
                                    od_param = [param3 + 1]
                            ...
                            priv_idx_train = [i for i, val in enumerate(sensitive_processed) if val == 1]
                            unpriv_idx_train = [i for i, val in enumerate(sensitive_processed) if val == 0]

                            metric_evaluator = MetricEvaluator(self.metric_type)
                            outc = metric_evaluator.compute(
                                y_true=y_processed,
                                y_pred=y_pred,
                                priv_idx=priv_idx_train,
                                unpriv_idx=unpriv_idx_train
                            )
...
pipeline_df, coefs, coef_rank = executor.run_pipeline(filename_train, X_train, y_train, sensitive_attr_train)
print(pipeline_df)
print(coefs)
print(coef_rank)
