...
class Imputer():
    ...
    * exclude: str, default = 'None' name of variable to be excluded
    ...
    def KNN_imputation(self, dataset, k):
        ...
        df = dataset
        # print(df)
        # imputed_X = KNNImputer(n_neighbors=k).fit_transform(dataset)
        if dataset.select_dtypes(['number']).isnull().sum().sum() > 0:
            
            X = dataset.select_dtypes(['number'])
            # print(X.columns)
            X = KNN(k=k, verbose=False).fit_transform(X)
            # for i in X.columns:

            #     X[i] = KNN(k=k, verbose=False).fit_transform(X)
            #     # X[i] = KNNImputer(n_neighbors=k).fit_transform(X)
            ...
            pass
        # if isinstance(imputed_X, np.ndarray):
        #     imputed_X = pd.DataFrame(data=imputed_X,columns=df.columns)
        return df
        return imputed_X
