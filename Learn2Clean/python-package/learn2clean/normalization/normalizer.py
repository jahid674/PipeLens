...
class Normalizer():
    ...
    * threshold: float, default =  None
    ...
    def MA_normalization(self, dataset):
        # Normalize numeric columns with maxAbsolute scaler normalisation

        d = dataset

        if self.verbose:

            print("Max Absolute Scaler normalizing... ")
    ...
    def None_normalization(self, dataset):
        d = dataset

        if self.verbose:

            print("None normalizing... ")
        ...
        for key in ['train']:
            ...
            if (not isinstance(self.dataset[key], dict)):
                ...
                if (self.strategy == "DS"):
                    ...
                    dn = self.MA_normalization(d)
                
                elif (self.strategy == "NONE_normalization"):

                    dn = self.None_normalization(d)
                
                elif (self.strategy == "SS"):

                    dn = self.SS_normalization(d)
                
                elif (self.strategy == "RS"):

                    dn = self.RS_normalization(d)
    ...
    def SS_normalization(self, dataset):
        # Normalize numeric columns with maxAbsolute scaler normalisation
        ...
        if self.verbose:
            ...
            print("Standard Scaler normalizing... ")
        ...
        if (type(dataset) != pd.core.series.Series):
            ...
            for column in X.columns:
                # if (X[column].isnull().values.any() == False):
                #     # print(statistics.stdev(X[column]))
                X[column] = StandardScaler().fit_transform(X[column].to_frame())
                    # X[column] = (X[column] - X[column].mean())/statistics.stdev(X[column])
    ...
    def RS_normalization(self, dataset):
        # Normalize numeric columns with maxAbsolute scaler normalisation
        ...
        if self.verbose:
            ...
            print("Robust Scaler normalizing... ")
        ...
        if (type(dataset) != pd.core.series.Series):
            ...
            for column in X.columns:
                X[column] = RobustScaler().fit_transform(X[column].to_frame())
