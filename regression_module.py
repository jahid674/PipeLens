...
dataset = 'housing'
modelType = 'reg' #'lr' # 'nb' Logistic Regression or Gaussian Naive Bayes
metric_type = 'rmse'
algo_type = '2step'
...
#       data_final[column] = le.fit_transform(data_final[column]) 
    ...
    for column in data_final.columns:
        ...
        name = column
        key = 'corr_' + name
        profile[key]= round(corr*scaling_factor,5)
        i+=1
   
    return list(profile.values()),list(profile.keys())
...
#     logging.warning(missing_data)
    missing_colmn_categorical = train.select_dtypes(include=['object']).columns
    missing_colmn_numerical = train.select_dtypes(include=['int','float']).columns
...
class base:
        ...
        def optimize_projection(self, init_params, f_goal):
        ...
        def f_score_look_up2(self,profiles_df,elem):
                if(modelType=='reg'):
                        ...
                        try:
                                return profiles_df.loc[(profiles_df[column_names[0]] == elem[0]) & (profiles_df[column_names[1]] == elem[1] ) 
                                                & (profiles_df[column_names[2]] == elem[2]) & (profiles_df[column_names[3]] == elem[3]) 
        ...
        def optimize_2_step(self, init_params, f_goal):
        ...
        def create_historic_data(self,file_name):
                ...
                if not(os.path.exists(file_name)):
                        ...
                        model_params = len(model_strategy)
                                ...
                                for param2 in range(norm_params):
                                        for param3 in range(od_params):
                                                ...
                                                # print("Dropping missing values ...")
                                                        ...
                                                        elif modelType=='reg' and model_strategy[param4]=='regression':
                                                                model_param=[1]
                                                                updated_model = Regression().generate_regression(outlier_X_train,outlier_y_train)
                                                        elif modelType=='reg' and model_strategy[param4]=='svm':
                                                                model_param=[2]
                                                                updated_model = SVR(kernel = 'rbf').fit(outlier_X_train,outlier_y_train)
                                                        ...
                                                        profile_gen,profile_keys = p.populate_profiles(pd.concat([outlier_X_train, outlier_y_train], axis=1),fraction_out)
                        ...
                        param_lst_df = pd.DataFrame(param_lst, columns= self.param_columns   + ['out_before_out_strat','out_before_norm_strat','profile_median']+profile_keys+["fairness"])
                        param_lst_df.to_csv(file_name, index=False)
                else :
                        param_lst_df = pd.read_csv(file_name)

                if(dataset=='hmda'):
                        profile_keys = ['corr_race', 'corr_gender',  'corr_loan_type',  'corr_applicant_age',  'corr_lien_status',  'corr_LV', 'corr_DI', 'corr_income_brackets']
                elif dataset=='housing':
                       #profile_keys = ['profile_median','corr_MSSubClass', 'corr_Neighborhood', 'corr_OverallQual',  'corr_YearBuilt',  'corr_ExterQual', 'corr_BsmtQual', 'corr_TotalBsmtSF',  'corr_1stFlrSF',  'corr_GrLivArea',   'corr_KitchenQual',  'corr_GarageYrBlt', 'ot_GarageYrBlt', 'corr_GarageCars',  'corr_GarageArea']
                       #profile_keys = ['corr_MSSubClass', 'corr_Neighborhood',  'corr_OverallQual', 'corr_YearBuilt', 'corr_ExterQual', 'corr_BsmtQual',  'corr_TotalBsmtSF',  'corr_1stFlrSF', 'corr_GrLivArea',  'corr_FullBath','corr_KitchenQual',  'corr_GarageYrBlt','corr_GarageCars', 'corr_GarageArea' ]
                #        profile_keys = ['out_before_out_strat','out_before_norm_strat','profile_median','corr_MSSubClass','ot_MSSubClass','corr_LotFrontage','ot_LotFrontage','corr_LotArea','ot_LotArea','corr_Neighborhood','ot_Neighborhood','corr_OverallQual','ot_OverallQual','corr_YearBuilt','ot_YearBuilt','corr_YearRemodAdd','ot_YearRemodAdd','corr_ExterQual','ot_ExterQual','corr_Foundation','ot_Foundation','corr_BsmtQual','ot_BsmtQual','corr_BsmtFinType1','ot_BsmtFinType1','corr_BsmtFinSF1','ot_BsmtFinSF1','corr_TotalBsmtSF','ot_TotalBsmtSF','corr_HeatingQC','ot_HeatingQC','corr_1stFlrSF','ot_1stFlrSF','corr_2ndFlrSF','ot_2ndFlrSF','corr_GrLivArea','ot_GrLivArea','corr_FullBath','ot_FullBath','corr_KitchenQual','ot_KitchenQual','corr_TotRmsAbvGrd','ot_TotRmsAbvGrd','corr_Fireplaces','ot_Fireplaces','corr_GarageYrBlt','ot_GarageYrBlt','corr_GarageFinish','ot_GarageFinish','corr_GarageCars','ot_GarageCars','corr_GarageArea','ot_GarageArea']
                       profile_keys = ['profile_median','corr_MSSubClass','corr_Neighborhood','corr_OverallQual','corr_YearBuilt','corr_YearRemodAdd','corr_ExterQual','corr_BsmtQual','corr_TotalBsmtSF','corr_1stFlrSF','corr_2ndFlrSF','corr_GrLivArea','corr_KitchenQual','corr_GarageYrBlt','corr_GarageCars','corr_GarageArea']
                elif dataset=='adult':
                #        profile_keys = ["diff_sensitive_attr","ratio_sensitive_attr","class_imbalance_ratio"]
                       profile_keys = ["diff_sensitive_attr","ratio_sensitive_attr","class_imbalance_ratio","corr_Age","corr_Workclass","corr_Education","corr_Martial_Status","corr_Occupation","corr_Relationship","corr_Race","corr_Sex","corr_Hours_per_week"]
                self.profiles = profile_keys+[ 'out_before_out_strat','out_before_norm_strat']
                ...
                elif (algo_type == '1step'):
                        param_lst_df = param_lst_df[['missing_value','normalization','outlier','model','fairness']]
                        t = StandardScaler().fit(param_lst_df).transform(param_lst_df)
                        param_lst_df = pd.DataFrame(data=t,columns=param_lst_df.columns)
                                        ...
                                        for param3 in range(od_params):
                                                ...
                                                # print("Dropping missing values ...")
                                                        ...
                                                        if modelType == 'lr':
                                                                ...
                                                                model_param=[2]
                                                                updated_model = SVR(kernel = 'rbf').fit(outlier_X_test,outlier_y_test)
                                                        ...
                                                        param_lst.append(mv_param + norm_param + od_param +model_param+ f)
...
filename_test = 'historical_data/historical_data_test_model_component_'+modelType+'_'+metric_type+'_'+dataset+'.csv'
filename_train = 'historical_data/historical_data_train_model_component_'+modelType+'_'+metric_type+'_'+dataset+'.csv'
...
#        f_goals = [0.055]
        ...
        # f_goals = [23100,24000,25000,26000,27000,28000,29000,30000,31000,32000,33000,34000,35000]
...
f = sys.stdout
metric_path  = 'metric/metric_regresssion_'+algo_type+"_"+modelType+'_'+metric_type+'_'+dataset+'.csv'
...
for f_goal in f_goals:
        ...
        for seed_ in gg:
                ...
                if(algo_type=='projection'):
                  p.optimize_projection(seed_, f_goal)
                elif (algo_type=='2step'):
                  p.optimize_2_step(seed_, f_goal)
                elif (algo_type=='1step'):
